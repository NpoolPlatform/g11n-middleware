...
func (alc *AppLangCreate) defaults() error {
	...
	if _, ok := alc.mutation.EntID(); !ok {
		if applang.DefaultEntID == nil {
			return fmt.Errorf("ent: uninitialized applang.DefaultEntID (forgotten import ent/runtime?)")
		}
...
func (alc *AppLangCreate) check() error {
	...
	if _, ok := alc.mutation.EntID(); !ok {
		return &ValidationError{Name: "ent_id", err: errors.New(`ent: missing required field "AppLang.ent_id"`)}
	}
	return nil
}
