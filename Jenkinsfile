  ...
  stages {
    ...
    stage('Release docker image for feature') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          feature_name=`echo $BRANCH_NAME | awk -F '/' '{ print $2 }'`
          set +e
          docker images | grep g11n-middleware | grep $feature_name
          rc=$?
          set -e
          if [ 0 -eq $rc ]; then
    ...
    stage('Release docker image for development') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          set +e
          docker images | grep g11n-middleware | grep latest
          rc=$?
          set -e
          if [ 0 -eq $rc ]; then
    ...
    stage('Deploy for feature') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          feature_name=`echo $BRANCH_NAME | awk -F '/' '{ print $2 }'`
          sed -i "s/g11n-middleware:latest/g11n-middleware:$feature_name/g" cmd/g11n-middleware/k8s/02-g11n-middleware.yaml
    ...
    stage('Deploy for development') {
      ...
      steps {
        sh 'sed -i "s/uhub.service.ucloud.cn/$DOCKER_REGISTRY/g" cmd/g11n-middleware/k8s/02-g11n-middleware.yaml'
